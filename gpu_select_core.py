...
VERSION = "3.3.0"
APP_NAME = "GGS-Select"
APP_FULL_NAME = "GGS-Select v3.0 — СППР для выбора газопоршневых установок"
...
SANCTION_SCENARIOS = {
    "Оптимистичный": {"k_geopol": 0.15, "probability": 0.15,
                      "description": "Нормализация отношений, снятие части санкций"},
    "Базовый":       {"k_geopol": 0.60, "probability": 0.60,
                      "description": "Сохранение текущего санкционного режима"},
    "Пессимистичный":{"k_geopol": 0.95, "probability": 0.25,
...
def calculate_lcc(gpu: GPUData, params: LCCParams) -> LCCResult:
    ...
    if h > 0:
        ...
        while t_kr <= T:
            c_overhaul_total += overhaul_cost_per_kr * n / (1 + r) ** t_kr
            t_kr += kr_interval_years
    ...
    # 7. Персонал
    c_staff_total = sum(
        params.staff_cost_per_year / (1 + r) ** t for t in range(1, T + 1)
    )
...
def get_raw_values(
    ...
    lcc = calculate_lcc(gpu, lcc_params)
    spec_lcc = calculate_specific_lcc(lcc, gpu, lcc_params)

    # Число установок для целевой мощности
    n_units = calculate_num_units(gpu, target_power_kw)
    ...
    # v3.3: ИСПРАВЛЕН баг — ранее K2.2 сравнивал USD/ч, CNY/ч и RUB/ч напрямую
    ...
    maint_rub_per_h = gpu.maintenance_usd_per_h * maint_fx
    ...
    return {
        ...
        "K2.2": maint_rub_per_h,  # v3.3: теперь в руб/ч (единая валюта)
        "K2.3": spec_lcc,
        "K2.4": currency_risk,
...
def monte_carlo_analysis(
    ...
    for sim in range(n_simulations):
        ...
        # Цена газа: нормальное распределение
        params.gas_price_rub = max(3000, rng.normal(7500, 1500))

        # Ставка дисконтирования: треугольное распределение
        params.discount_rate = rng.triangular(0.08, 0.12, 0.16)
...
def calculate_financial(
    ...
    for t in range(1, T + 1):
        rev_t = annual_revenue * (1 + 0.03) ** (t - 1)
        opex_t = annual_opex * (1 + 0.02) ** (t - 1)
        cf = rev_t - opex_t
...
def run_full_analysis(
    ...
    # Этап 4: СЖЦ (per-unit)
    lcc_results = {name: calculate_lcc(gpu, params) for name, gpu in gpus.items()}
    specific_lcc = {
        name: round(calculate_specific_lcc(lcc, gpus[name], params), 2)
        for name, lcc in lcc_results.items()
    }

    # Этап 4б: СЖЦ (station-level для целевой мощности)
    station_lcc = {
        name: calculate_station_lcc(gpu, params, target_power_kw)
        for name, gpu in gpus.items()
    }
