"""
GGS-Select v3.3: Интерактивный веб-интерфейс СППР для многокритериального выбора газопоршневых установок
Полная переделка с тёмной темой, Plotly графиками и современным UI/UX дизайном
...
COLORS = {
    ...
    'cluster_western': '#ef4444',
    'cluster_chinese': '#10b981',
    'cluster_russian': '#3b82f6',
}
...
st.set_page_config(
    page_title=f"Выбери ГПУ v{VERSION} | СППР выбора ГПУ",
    page_icon="⚡",
    layout="wide",
...
with tab1:
    ...
    with col_radar:
        ...
        radar_colors = [COLORS['primary'], '#06b6d4', '#10b981']
        group_keys = list(GROUP_WEIGHTS_BY_CATEGORY.get(category, {}).keys())
        group_labels = [g.replace("G1: ", "").replace("G2: ", "").replace("G3: ", "").replace("G4: ", "").replace("G5: ", "") for g in group_keys]
        # Shorter labels
        short_labels = []
        for gl in group_labels:
            if 'Техни' in gl: short_labels.append('Технические')
            elif 'Эконом' in gl: short_labels.append('Экономика')
            elif 'Эксплуат' in gl: short_labels.append('Эксплуатация')
            elif 'Экол' in gl: short_labels.append('Экология')
            elif 'Санкц' in gl: short_labels.append('Санкции')
            else: short_labels.append(gl[:12])
        ...
        for idx, gpu_name in enumerate(top3_names):
            ...
            fahp_val = analysis_results.fahp_scores.get(gpu_name, 0)
            spec_lcc = analysis_results.specific_lcc.get(gpu_name, 0)

            # Normalized radar values for 5 groups
            ...
            eff_norm = gpu_data.efficiency_el / max(all_eff) if max(all_eff) > 0 else 0
            # Economic: lower LCC is better
            lcc_norm = 1 - (spec_lcc / max(all_lcc)) if max(all_lcc) > 0 else 0
            # Operational: resource + load speed
            op_norm = min(1.0, (gpu_data.resource_to_overhaul / 80.0 + gpu_data.load_speed / 120.0) / 2)
            # Environmental
            env_max_nox = max(g.nox_emissions for g in gpus.values()) if gpus else 500
            env_norm = 1 - (gpu_data.nox_emissions / env_max_nox) if env_max_nox > 0 else 0
            # Sanctions (KSU)
            ksu_max = max(all_ksu) if all_ksu else 1
            san_norm = ksu_val / ksu_max if ksu_max > 0 else 0

            values = [eff_norm, max(0.1, lcc_norm), op_norm, max(0.1, env_norm), san_norm]
            ...
            fig_radar.add_trace(go.Scatterpolar(
                ...
                fill='toself',
                fillcolor=f'rgba{tuple(list(int(radar_colors[idx].lstrip("#")[j:j+2], 16) for j in (0,2,4)) + [0.08])}',
            ))
...
">
    ...
    <div style="text-align:center;color:{COLORS['text_secondary']};font-size:10px;margin-bottom:4px;">
        СППР «Выбери ГПУ» v{VERSION}  ·  {APP_AUTHOR}  ·  {APP_AFFILIATION}  ·  Кафедра ТЭС  ·  2026
    </div>
